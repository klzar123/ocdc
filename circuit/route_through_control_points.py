...
def _get_angle_out(pos1, pos2, angle_in):
    """Returns an output angle for a manhattan route as a function of pos1, pos2 and input angle.
...
class RouteManhattanControlPoints(RouteManhattanBasic):
    """Manhattan route that bends through a list of control points.
    The route generated by RouteManhattanControlPoints will have the following properties:
    - The route is a regular manhattan route.
    - The route bends through the control_points in the order same order as in control_points.
    ...
    def __example1(cls):
        ...
        from picazzo3.circuit.place_route import PlaceComponents
        from ipkiss3 import all as i3
        ...
        circuit_layout = circuit.Layout(child_transformations={'gr': (0, 0),
                                                               'grb1': (-100, 0),
                                                               'grb2': i3.Rotation(rotation=180.0) + i3.Translation(
                                                                   (+100, 0))})
        ...
        class BondPad(i3.PCell):
            ...
            class Layout(i3.LayoutView):
                ...
                def _generate_ports(self, ports):
                    ports += i3.ElectricalPort(name="m1", position=(0.0, 0.0),
                                               shape=self.size, process=self.metal_layer.process)
                    return ports
    ...
    def _get_used_control_point_in_out_angles(self):
        ...
        for cnt, wp in enumerate(self.control_points[1:], 1):
            ...
            out_angles.append(_get_angle_out(
                used_start_control_points[-1], used_stop_control_points[-1], in_angles[-1]))
    ...
    def _default_points(self, pts):
        ...
        if len(self.control_points) == 0:
            ...
            used_start_control_points, in_angles, used_stop_control_points, out_angles =\
                self._get_used_control_point_in_out_angles()
            pts.append(self.input_port.position)
            for cnt, (start_wp, ia, stop_wp, oa) in enumerate(
                    zip(used_start_control_points, in_angles, used_stop_control_points, out_angles)):
